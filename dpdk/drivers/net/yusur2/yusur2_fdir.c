...
#define YUSUR2_DEFAULT_FLEXBYTES_OFFSET  12 /* default flexbytes offset in bytes */
#define YUSUR2_FDIR_MAX_FLEX_LEN         2 /* len in bytes of flexbytes */
#define YUSUR2_MAX_FLX_SOURCE_OFF        62
#define YUSUR2_FDIRCTRL_FLEX_MASK        (0x1F << YUSUR2_FDIRCTRL_FLEX_SHIFT)
#define YUSUR2_FDIRCMD_CMD_INTERVAL_US   10

#define YUSUR2_FDIR_FLOW_TYPES ( \
...
#define YUSUR2_FDIRIP6M_INNER_MAC_SHIFT 4

static int fdir_erase_filter_82599(struct yusur2_hw *hw, uint32_t fdirhash);
...
static int yusur2_set_fdir_flex_conf(struct rte_eth_dev *dev,
		const struct rte_eth_fdir_flex_conf *conf, uint32_t *fdirctrl);
static int fdir_enable_82599(struct yusur2_hw *hw, uint32_t fdirctrl);
static int yusur2_fdir_filter_to_atr_input(
		const struct rte_eth_fdir_filter *fdir_filter,
		union yusur2_atr_input *input,
		enum rte_fdir_mode mode);
static uint32_t yusur2_atr_compute_hash_82599(union yusur2_atr_input *atr_input,
				 uint32_t key);
static uint32_t atr_compute_sig_hash_82599(union yusur2_atr_input *input,
		enum rte_fdir_pballoc_type pballoc);
static uint32_t atr_compute_perfect_hash_82599(union yusur2_atr_input *input,
		enum rte_fdir_pballoc_type pballoc);
static int fdir_write_perfect_filter_82599(struct yusur2_hw *hw,
			union yusur2_atr_input *input, uint8_t queue,
			uint32_t fdircmd, uint32_t fdirhash,
			enum rte_fdir_mode mode);
static int fdir_add_signature_filter_82599(struct yusur2_hw *hw,
		union yusur2_atr_input *input, u8 queue, uint32_t fdircmd,
		uint32_t fdirhash);
static int yusur2_add_del_fdir_filter(struct rte_eth_dev *dev,
			      const struct rte_eth_fdir_filter *fdir_filter,
			      bool del,
			      bool update);
static int yusur2_fdir_flush(struct rte_eth_dev *dev);
static void yusur2_fdir_info_get(struct rte_eth_dev *dev,
			struct rte_eth_fdir_info *fdir_info);
static void yusur2_fdir_stats_get(struct rte_eth_dev *dev,
			struct rte_eth_fdir_stats *fdir_stats);

/**
 * This function is based on yusur2_fdir_enable_82599() in base/yusur2_82599.c.
 ...
 **/
static int
fdir_enable_82599(struct yusur2_hw *hw, uint32_t fdirctrl)
{
	int i;
	 ...
	 */
	fdirctrl |= (0xA << YUSUR2_FDIRCTRL_MAX_LENGTH_SHIFT) |
...
configure_fdir_flags(const struct rte_fdir_conf *conf, uint32_t *fdirctrl)
{
	*fdirctrl = 0;
	...
	case RTE_FDIR_PBALLOC_64K:
		...
		*fdirctrl |= YUSUR2_FDIRCTRL_PBALLOC_64K;
		break;
	case RTE_FDIR_PBALLOC_128K:
		/* 16k - 1 signature filters */
		*fdirctrl |= YUSUR2_FDIRCTRL_PBALLOC_128K;
		break;
	case RTE_FDIR_PBALLOC_256K:
		/* 32k - 1 signature filters */
		*fdirctrl |= YUSUR2_FDIRCTRL_PBALLOC_256K;
		break;
	default:
		/* bad value */
	...
	case RTE_FDIR_REPORT_STATUS:
		...
		*fdirctrl |= YUSUR2_FDIRCTRL_REPORT_STATUS;
		break;
	case RTE_FDIR_REPORT_STATUS_ALWAYS:
		/* always report status */
		*fdirctrl |= YUSUR2_FDIRCTRL_REPORT_STATUS_ALWAYS;
		break;
	default:
		/* bad value */
	...
	*fdirctrl |= (YUSUR2_DEFAULT_FLEXBYTES_OFFSET / sizeof(uint16_t)) <<
		     YUSUR2_FDIRCTRL_FLEX_SHIFT;

	if (conf->mode >= RTE_FDIR_MODE_PERFECT &&
	    conf->mode <= RTE_FDIR_MODE_PERFECT_TUNNEL) {
		*fdirctrl |= YUSUR2_FDIRCTRL_PERFECT_MATCH;
		*fdirctrl |= (conf->drop_queue << YUSUR2_FDIRCTRL_DROP_Q_SHIFT);
		if (conf->mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN)
			*fdirctrl |= (YUSUR2_FDIRCTRL_FILTERMODE_MACVLAN
...
/*
 * This references yusur2_fdir_set_input_mask_82599() in base/yusur2_82599.c,
...
fdir_set_input_mask_82599(struct rte_eth_dev *dev)
{
	...
	struct yusur2_hw_fdir_info *info =
			YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	/*
	 * mask VM pool and DIPv6 since there are currently not supported
	 * mask FLEX byte, it will be set in flex_conf
	 */
	uint32_t fdirm = YUSUR2_FDIRM_POOL | YUSUR2_FDIRM_DIPv6;
	...
	if (info->mask.vlan_tci_mask == rte_cpu_to_be_16(0x0FFF))
		...
		fdirm |= YUSUR2_FDIRM_VLANID | YUSUR2_FDIRM_VLANP;
	 ...
	 * can not use YUSUR2_WRITE_REG.
	 */
	reg = YUSUR2_PCI_REG_ADDR(hw, YUSUR2_FDIRSIP4M);
	*reg = ~(info->mask.src_ipv4_mask);
	reg = YUSUR2_PCI_REG_ADDR(hw, YUSUR2_FDIRDIP4M);
	*reg = ~(info->mask.dst_ipv4_mask);
	...
	if (dev->data->dev_conf.fdir_conf.mode == RTE_FDIR_MODE_SIGNATURE) {
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_FDIRIP6M, ~fdiripv6m);
	}
...
fdir_set_input_mask_x550(struct rte_eth_dev *dev)
{
	...
	if (mode == RTE_FDIR_MODE_PERFECT_TUNNEL)
		YUSUR2_WRITE_REG(hw, YUSUR2_VXLANCTRL, RTE_VXLAN_DEFAULT_PORT);

	/* some bits must be set for mac vlan or tunnel mode */
	fdirm |= YUSUR2_FDIRM_L4P | YUSUR2_FDIRM_L3P;
	...
	fdiripv6m = ((u32)0xFFFFU << YUSUR2_FDIRIP6M_DIPM_SHIFT);
	fdiripv6m |= YUSUR2_FDIRIP6M_ALWAYS_MASK;
	if (mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN)
		fdiripv6m |= YUSUR2_FDIRIP6M_TUNNEL_TYPE |
				YUSUR2_FDIRIP6M_TNI_VNI;

	if (mode == RTE_FDIR_MODE_PERFECT_TUNNEL) {
		fdiripv6m |= YUSUR2_FDIRIP6M_INNER_MAC;
		mac_mask = info->mask.mac_addr_byte_mask &
			(YUSUR2_FDIRIP6M_INNER_MAC >>
			YUSUR2_FDIRIP6M_INNER_MAC_SHIFT);
		fdiripv6m &= ~((mac_mask << YUSUR2_FDIRIP6M_INNER_MAC_SHIFT) &
				YUSUR2_FDIRIP6M_INNER_MAC);
		...
		case 0x0:
			/* Mask vxlan id */
			fdiripv6m |= YUSUR2_FDIRIP6M_TNI_VNI;
			break;
		case 0x00FFFFFF:
			fdiripv6m |= YUSUR2_FDIRIP6M_TNI_VNI_24;
			break;
		case 0xFFFFFFFF:
			break;
		default:
	...
	YUSUR2_WRITE_REG(hw, YUSUR2_FDIRDIP4M, 0xFFFFFFFF);
	YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIP4M, 0xFFFFFFFF);

	return YUSUR2_SUCCESS;
}

static int
yusur2_fdir_store_input_mask_82599(struct rte_eth_dev *dev,
				  const struct rte_eth_fdir_masks *input_mask)
{
	struct yusur2_hw_fdir_info *info =
		YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	...
	memset(&info->mask, 0, sizeof(struct yusur2_hw_fdir_mask));
...
static int
yusur2_fdir_store_input_mask_x550(struct rte_eth_dev *dev,
				 const struct rte_eth_fdir_masks *input_mask)
{
	struct yusur2_hw_fdir_info *info =
		YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
...
static int
yusur2_fdir_store_input_mask(struct rte_eth_dev *dev,
			    const struct rte_eth_fdir_masks *input_mask)
{
	enum rte_fdir_mode mode = dev->data->dev_conf.fdir_conf.mode;
	...
	if (mode >= RTE_FDIR_MODE_SIGNATURE &&
		...
		return yusur2_fdir_store_input_mask_82599(dev, input_mask);
	else if (mode >= RTE_FDIR_MODE_PERFECT_MAC_VLAN &&
		 mode <= RTE_FDIR_MODE_PERFECT_TUNNEL)
		return yusur2_fdir_store_input_mask_x550(dev, input_mask);
...
int
yusur2_fdir_set_input_mask(struct rte_eth_dev *dev)
{
	enum rte_fdir_mode mode = dev->data->dev_conf.fdir_conf.mode;
...
int
yusur2_fdir_set_flexbytes_offset(struct rte_eth_dev *dev,
				uint16_t offset)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t fdirctrl;
	...
	fdirctrl &= ~YUSUR2_FDIRCTRL_FLEX_MASK;
		    ...
		    const struct rte_eth_fdir_masks *input_mask)
{
	...
	ret = yusur2_fdir_store_input_mask(dev, input_mask);
	if (ret)
		return ret;

	return yusur2_fdir_set_input_mask(dev);
}

/*
 * yusur2_check_fdir_flex_conf -check if the flex payload and mask configuration
 * arguments are valid
 */
static int
yusur2_set_fdir_flex_conf(struct rte_eth_dev *dev,
		const struct rte_eth_fdir_flex_conf *conf, uint32_t *fdirctrl)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_hw_fdir_info *info =
	...
	fdirm = YUSUR2_READ_REG(hw, YUSUR2_FDIRM);
	...
	for (i = 0; i < conf->nb_payloads; i++) {
		...
		if (((flex_cfg->src_offset[0] & 0x1) == 0) &&
		    ...
		    (flex_cfg->src_offset[0] <= YUSUR2_MAX_FLX_SOURCE_OFF)) {
			*fdirctrl &= ~YUSUR2_FDIRCTRL_FLEX_MASK;
			*fdirctrl |=
	...
	for (i = 0; i < conf->nb_flexmasks; i++) {
		...
		if (flexbytes == UINT16_MAX)
			...
			/* YUSUR2_FDIRM_FLEX is set by default when set mask */
	...
	info->flex_bytes_offset = (uint8_t)((*fdirctrl &
					    YUSUR2_FDIRCTRL_FLEX_MASK) >>
					    YUSUR2_FDIRCTRL_FLEX_SHIFT);
	return 0;
}

int
yusur2_fdir_configure(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	int err;
	...
	YUSUR2_WRITE_REG(hw, YUSUR2_RXPBSIZE(0),
	    (YUSUR2_READ_REG(hw, YUSUR2_RXPBSIZE(0)) - pbsize));
	...
	err = yusur2_set_fdir_flex_conf(dev,
...
/*
 * Convert DPDK rte_eth_fdir_filter struct to yusur2_atr_input union that is used
 * by the YUSUR2 driver code.
 */
static int
yusur2_fdir_filter_to_atr_input(const struct rte_eth_fdir_filter *fdir_filter,
		union yusur2_atr_input *input, enum rte_fdir_mode mode)
{
	...
	case RTE_ETH_FLOW_NONFRAG_IPV4_UDP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_UDPV4;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV4_TCP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_TCPV4;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV4_SCTP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_SCTPV4;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV4_OTHER:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_IPV4;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV6_UDP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_UDPV6;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV6_TCP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_TCPV6;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV6_SCTP:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_SCTPV6;
		break;
	case RTE_ETH_FLOW_NONFRAG_IPV6_OTHER:
		input->formatted.flow_type = YUSUR2_ATR_FLOW_TYPE_IPV6;
		break;
	default:
		break;
	}
	...
	case RTE_ETH_FLOW_NONFRAG_IPV4_TCP:
		...
		/* fall-through */
	/*for SCTP flow type, port and verify_tag are meaningless in yusur2.*/
	case RTE_ETH_FLOW_NONFRAG_IPV4_SCTP:
	case RTE_ETH_FLOW_NONFRAG_IPV4_OTHER:
...
/*
 * The below function is taken from the FreeBSD YUSUR2 drivers release
 * 2.3.8. The only change is not to mask hash_result with YUSUR2_ATR_HASH_MASK
 ...
 *
 * Note that the bkt_hash field in the yusur2_atr_input structure is also never
 * set.
 *
 ...
 **/
static uint32_t
yusur2_atr_compute_hash_82599(union yusur2_atr_input *atr_input,
				 uint32_t key)
{
	/*
	...
	flow_vm_vlan = YUSUR2_NTOHL(atr_input->dword_stream[0]);

	/* generate common hash dword */
	for (i = 1; i <= 13; i++)
		common_hash_dword ^= atr_input->dword_stream[i];

	hi_hash_dword = YUSUR2_NTOHL(common_hash_dword);
...
static uint32_t
atr_compute_perfect_hash_82599(union yusur2_atr_input *input,
		enum rte_fdir_pballoc_type pballoc)
{
	if (pballoc == RTE_FDIR_PBALLOC_256K)
		return yusur2_atr_compute_hash_82599(input,
				YUSUR2_ATR_BUCKET_HASH_KEY) &
 ...
 */
static inline int
yusur2_fdir_check_cmd_complete(struct yusur2_hw *hw, uint32_t *fdircmd)
{
	int i;
	...
	for (i = 0; i < YUSUR2_FDIRCMD_CMD_POLL; i++) {
		*fdircmd = YUSUR2_READ_REG(hw, YUSUR2_FDIRCMD);
		if (!(*fdircmd & YUSUR2_FDIRCMD_CMD_MASK))
			return 0;
		rte_delay_us(YUSUR2_FDIRCMD_CMD_INTERVAL_US);
	}
 ...
 * yusur2_atr_compute_sig_hash_82599(). However that can't be used here as it
 * doesn't support calculating a hash for an IPv6 filter.
 */
static uint32_t
atr_compute_sig_hash_82599(union yusur2_atr_input *input,
		enum rte_fdir_pballoc_type pballoc)
{
	uint32_t bucket_hash, sig_hash;

	if (pballoc == RTE_FDIR_PBALLOC_256K)
		bucket_hash = yusur2_atr_compute_hash_82599(input,
				YUSUR2_ATR_BUCKET_HASH_KEY) &
	...
	sig_hash = yusur2_atr_compute_hash_82599(input,
			YUSUR2_ATR_SIGNATURE_HASH_KEY);

	return (sig_hash << YUSUR2_FDIRHASH_SIG_SW_INDEX_SHIFT) | bucket_hash;
}

/*
 * This is based on yusur2_fdir_write_perfect_filter_82599() in
 * base/yusur2_82599.c, with the ability to set extra flags in FDIRCMD register
 * added, and IPv6 support also added. The hash value is also pre-calculated
 * as the pballoc value is needed to do it.
 */
static int
fdir_write_perfect_filter_82599(struct yusur2_hw *hw,
			union yusur2_atr_input *input, uint8_t queue,
			uint32_t fdircmd, uint32_t fdirhash,
			enum rte_fdir_mode mode)
{
	...
	if (mode == RTE_FDIR_MODE_PERFECT) {
		 ...
		 * can not use YUSUR2_WRITE_REG.
		 */
		reg = YUSUR2_PCI_REG_ADDR(hw, YUSUR2_FDIRIPSA);
		*reg = input->formatted.src_ip[0];
		reg = YUSUR2_PCI_REG_ADDR(hw, YUSUR2_FDIRIPDA);
		*reg = input->formatted.dst_ip[0];
		...
		if (mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN) {
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(0), addr_low);
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(1), addr_high);
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(2), 0);
		} else {
			/* tunnel mode */
			...
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(0), addr_low);
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(1), tunnel_type);
			YUSUR2_WRITE_REG(hw, YUSUR2_FDIRSIPv6(2),
					input->formatted.tni_vni);
		}
		YUSUR2_WRITE_REG(hw, YUSUR2_FDIRIPSA, 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_FDIRIPDA, 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_FDIRPORT, 0);
	}
	...
	fdircmd |= YUSUR2_FDIRCMD_CMD_ADD_FLOW |
		  YUSUR2_FDIRCMD_LAST | YUSUR2_FDIRCMD_QUEUE_EN;
	fdircmd |= input->formatted.flow_type << YUSUR2_FDIRCMD_FLOW_TYPE_SHIFT;
	fdircmd |= (uint32_t)queue << YUSUR2_FDIRCMD_RX_QUEUE_SHIFT;
	fdircmd |= (uint32_t)input->formatted.vm_pool << YUSUR2_FDIRCMD_VT_POOL_SHIFT;
...
/**
 * This function is based on yusur2_atr_add_signature_filter_82599() in
 * base/yusur2_82599.c, but uses a pre-calculated hash value. It also supports
 ...
 **/
static int
fdir_add_signature_filter_82599(struct yusur2_hw *hw,
		union yusur2_atr_input *input, u8 queue, uint32_t fdircmd,
		uint32_t fdirhash)
{
	int err = 0;
...
/*
 * This is based on yusur2_fdir_erase_perfect_filter_82599() in
 * base/yusur2_82599.c. It is modified to take in the hash as a parameter so
 * that it can be used for removing signature and perfect filters.
 */
static int
fdir_erase_filter_82599(struct yusur2_hw *hw, uint32_t fdirhash)
{
	uint32_t fdircmd = 0;
	int err = 0;
...
static inline struct yusur2_fdir_filter *
yusur2_fdir_filter_lookup(struct yusur2_hw_fdir_info *fdir_info,
			 union yusur2_atr_input *key)
{
	int ret;
...
static inline int
yusur2_insert_fdir_filter(struct yusur2_hw_fdir_info *fdir_info,
			 struct yusur2_fdir_filter *fdir_filter)
{
	int ret;

	ret = rte_hash_add_key(fdir_info->hash_handle,
			       &fdir_filter->yusur2_fdir);
...
static inline int
yusur2_remove_fdir_filter(struct yusur2_hw_fdir_info *fdir_info,
			 union yusur2_atr_input *key)
{
	int ret;
	struct yusur2_fdir_filter *fdir_filter;
...
static int
yusur2_interpret_fdir_filter(struct rte_eth_dev *dev,
			    const struct rte_eth_fdir_filter *fdir_filter,
			    struct yusur2_fdir_rule *rule)
{
	...
	memset(rule, 0, sizeof(struct yusur2_fdir_rule));

	err = yusur2_fdir_filter_to_atr_input(fdir_filter,
					     &rule->yusur2_fdir,
					     fdir_mode);
	if (err)
		return err;
	...
	if (fdir_filter->action.behavior == RTE_ETH_FDIR_REJECT)
		rule->fdirflags = YUSUR2_FDIRCMD_DROP;
...
int
yusur2_fdir_filter_program(struct rte_eth_dev *dev,
			  struct yusur2_fdir_rule *rule,
			  bool del,
			  bool update)
{
	...
	struct yusur2_hw_fdir_info *info =
		YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	enum rte_fdir_mode fdir_mode = dev->data->dev_conf.fdir_conf.mode;
	struct yusur2_fdir_filter *node;
	bool add_node = FALSE;
	...
	if ((!del) &&
	    (hw->mac.type == yusur2_mac_X550 ||
	     hw->mac.type == yusur2_mac_X550EM_x ||
	     hw->mac.type == yusur2_mac_X550EM_a) &&
	    (rule->yusur2_fdir.formatted.flow_type ==
	     YUSUR2_ATR_FLOW_TYPE_IPV4 ||
	     rule->yusur2_fdir.formatted.flow_type ==
	     YUSUR2_ATR_FLOW_TYPE_IPV6) &&
	...
	if (is_perfect) {
		if (rule->yusur2_fdir.formatted.flow_type &
		    YUSUR2_ATR_L4TYPE_IPV6_MASK) {
			PMD_DRV_LOG(ERR, "IPv6 is not supported in"
				    " perfect mode!");
			return -ENOTSUP;
		}
		fdirhash = atr_compute_perfect_hash_82599(&rule->yusur2_fdir,
							  dev->data->dev_conf.fdir_conf.pballoc);
		fdirhash |= rule->soft_id <<
			YUSUR2_FDIRHASH_SIG_SW_INDEX_SHIFT;
	} else
		fdirhash = atr_compute_sig_hash_82599(&rule->yusur2_fdir,
						      dev->data->dev_conf.fdir_conf.pballoc);

	if (del) {
		err = yusur2_remove_fdir_filter(info, &rule->yusur2_fdir);
		if (err < 0)
			return err;
	...
	fdircmd_flags = (update) ? YUSUR2_FDIRCMD_FILTER_UPDATE : 0;
	if (rule->fdirflags & YUSUR2_FDIRCMD_DROP) {
		if (is_perfect) {
			queue = dev->data->dev_conf.fdir_conf.drop_queue;
			fdircmd_flags |= YUSUR2_FDIRCMD_DROP;
		} else {
			PMD_DRV_LOG(ERR, "Drop option is not supported in"
				    " signature mode.");
			return -EINVAL;
		}
	} else if (rule->queue < YUSUR2_MAX_RX_QUEUE_NUM)
		queue = (uint8_t)rule->queue;
	else
		return -EINVAL;

	node = yusur2_fdir_filter_lookup(info, &rule->yusur2_fdir);
	if (node) {
		if (update) {
		...
		add_node = TRUE;
		node = rte_zmalloc("yusur2_fdir",
				   sizeof(struct yusur2_fdir_filter),
				   0);
		if (!node)
			return -ENOMEM;
		rte_memcpy(&node->yusur2_fdir,
				 &rule->yusur2_fdir,
				 sizeof(union yusur2_atr_input));
		node->fdirflags = fdircmd_flags;
		node->fdirhash = fdirhash;
		node->queue = queue;

		err = yusur2_insert_fdir_filter(info, node);
		if (err < 0) {
			rte_free(node);
			return err;
		}
	...
	if (is_perfect) {
		err = fdir_write_perfect_filter_82599(hw, &rule->yusur2_fdir,
						      queue, fdircmd_flags,
						      fdirhash, fdir_mode);
	} else {
		err = fdir_add_signature_filter_82599(hw, &rule->yusur2_fdir,
						      queue, fdircmd_flags,
						      fdirhash);
	}
	if (err < 0) {
		...
		if (add_node)
			(void)yusur2_remove_fdir_filter(info, &rule->yusur2_fdir);
	} else {
...
/* yusur2_add_del_fdir_filter - add or remove a flow diretor filter.
 ...
 */
static int
yusur2_add_del_fdir_filter(struct rte_eth_dev *dev,
			  const struct rte_eth_fdir_filter *fdir_filter,
			  bool del,
			  bool update)
{
	...
	err = yusur2_interpret_fdir_filter(dev, fdir_filter, &rule);

	if (err)
		return err;

	return yusur2_fdir_filter_program(dev, &rule, del, update);
}

static int
yusur2_fdir_flush(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_hw_fdir_info *info =
			YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	...
	ret = yusur2_reinit_fdir_tables_82599(hw);
	if (ret < 0) {
...
static void
yusur2_fdir_info_get(struct rte_eth_dev *dev, struct rte_eth_fdir_info *fdir_info)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_hw_fdir_info *info =
			YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	...
	fdirctrl = YUSUR2_READ_REG(hw, YUSUR2_FDIRCTRL);
	offset = ((fdirctrl & YUSUR2_FDIRCTRL_FLEX_MASK) >>
			YUSUR2_FDIRCTRL_FLEX_SHIFT) * sizeof(uint16_t);
	...
	fdir_info->max_flexpayload = YUSUR2_FDIR_MAX_FLEX_LEN;

	if (fdir_info->mode == RTE_FDIR_MODE_PERFECT_MAC_VLAN ||
	    fdir_info->mode == RTE_FDIR_MODE_PERFECT_TUNNEL)
		fdir_info->flow_types_mask[0] = 0ULL;
	else
		fdir_info->flow_types_mask[0] = YUSUR2_FDIR_FLOW_TYPES;
	...
	fdir_info->flex_payload_limit = YUSUR2_MAX_FLX_SOURCE_OFF;
...
static void
yusur2_fdir_stats_get(struct rte_eth_dev *dev, struct rte_eth_fdir_stats *fdir_stats)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_hw_fdir_info *info =
		YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	...
	reg = YUSUR2_READ_REG(hw, YUSUR2_FDIRFREE);
	info->collision = (uint16_t)((reg & YUSUR2_FDIRFREE_COLL_MASK) >>
				     YUSUR2_FDIRFREE_COLL_SHIFT);
	info->free = (uint16_t)((reg & YUSUR2_FDIRFREE_FREE_MASK) >>
				YUSUR2_FDIRFREE_FREE_SHIFT);

	reg = YUSUR2_READ_REG(hw, YUSUR2_FDIRLEN);
	info->maxhash = (uint16_t)((reg & YUSUR2_FDIRLEN_MAXHASH_MASK) >>
				   YUSUR2_FDIRLEN_MAXHASH_SHIFT);
	info->maxlen  = (uint8_t)((reg & YUSUR2_FDIRLEN_MAXLEN_MASK) >>
				  YUSUR2_FDIRLEN_MAXLEN_SHIFT);

	reg = YUSUR2_READ_REG(hw, YUSUR2_FDIRUSTAT);
	info->remove += (reg & YUSUR2_FDIRUSTAT_REMOVE_MASK) >>
		YUSUR2_FDIRUSTAT_REMOVE_SHIFT;
	info->add += (reg & YUSUR2_FDIRUSTAT_ADD_MASK) >>
		YUSUR2_FDIRUSTAT_ADD_SHIFT;

	reg = YUSUR2_READ_REG(hw, YUSUR2_FDIRFSTAT) & 0xFFFF;
	info->f_remove += (reg & YUSUR2_FDIRFSTAT_FREMOVE_MASK) >>
		YUSUR2_FDIRFSTAT_FREMOVE_SHIFT;
	info->f_add += (reg & YUSUR2_FDIRFSTAT_FADD_MASK) >>
		YUSUR2_FDIRFSTAT_FADD_SHIFT;
	...
	reg = YUSUR2_READ_REG(hw, YUSUR2_FDIRCTRL);
	max_num = (1 << (FDIRENTRIES_NUM_SHIFT +
...
/*
 * yusur2_fdir_ctrl_func - deal with all operations on flow director.
 * @dev: pointer to the structure rte_eth_dev
 * @filter_op:operation will be taken
 * @arg: a pointer to specific structure corresponding to the filter_op
 */
int
yusur2_fdir_ctrl_func(struct rte_eth_dev *dev,
			enum rte_filter_op filter_op, void *arg)
{
	...
	case RTE_ETH_FILTER_ADD:
		ret = yusur2_add_del_fdir_filter(dev,
	...
	case RTE_ETH_FILTER_FLUSH:
		ret = yusur2_fdir_flush(dev);
		break;
	case RTE_ETH_FILTER_INFO:
		yusur2_fdir_info_get(dev, (struct rte_eth_fdir_info *)arg);
		break;
	case RTE_ETH_FILTER_STATS:
		yusur2_fdir_stats_get(dev, (struct rte_eth_fdir_stats *)arg);
		break;
	default:
...
void
yusur2_fdir_filter_restore(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
int
yusur2_clear_all_fdir_filter(struct rte_eth_dev *dev)
{
	struct yusur2_hw_fdir_info *fdir_info =
		YUSUR2_DEV_PRIVATE_TO_FDIR_INFO(dev->data->dev_private);
	struct yusur2_fdir_filter *fdir_filter;
	struct yusur2_fdir_filter *filter_flag;
	int ret = 0;

	/* flush flow director */
	rte_hash_reset(fdir_info->hash_handle);
	memset(fdir_info->hash_map, 0,
	       sizeof(struct yusur2_fdir_filter *) * YUSUR2_MAX_FDIR_FILTER_NUM);
