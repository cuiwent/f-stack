...
/**
 * @file rte_pmd_yusur2.h
 * yusur2 PMD specific functions.
 *
 **/

#ifndef _PMD_YUSUR2_H_
#define _PMD_YUSUR2_H_
 ...
 */
int rte_pmd_yusur2_set_vf_vlan_anti_spoof(uint16_t port, uint16_t vf,
					 uint8_t on);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf, uint8_t on);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_set_vf_vlan_insert(uint16_t port, uint16_t vf,
		uint16_t vlan_id);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_set_tx_loopback(uint16_t port, uint8_t on);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_set_all_queues_drop_en(uint16_t port, uint8_t on);
 ...
 *   - (-ENOTSUP) if hardware doesn't support this feature.
 */
int rte_pmd_yusur2_macsec_enable(uint16_t port, uint8_t en, uint8_t rp);
 ...
 *   - (-ENOTSUP) if hardware doesn't support this feature.
 */
int rte_pmd_yusur2_macsec_disable(uint16_t port);
 ...
 *   - (-ENOTSUP) if hardware doesn't support this feature.
 */
int rte_pmd_yusur2_macsec_config_txsc(uint16_t port, uint8_t *mac);
 ...
 *   - (-ENOTSUP) if hardware doesn't support this feature.
 */
int rte_pmd_yusur2_macsec_config_rxsc(uint16_t port, uint8_t *mac, uint16_t pi);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_macsec_select_txsa(uint16_t port, uint8_t idx, uint8_t an,
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_macsec_select_rxsa(uint16_t port, uint8_t idx, uint8_t an,
		uint32_t pn, uint8_t *key);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_init(uint16_t port);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_state_show(uint16_t port, uint32_t *state);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_state_set(uint16_t port, uint32_t *new_state);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_event_show(uint16_t port,
				    uint32_t event,
				    uint32_t *state);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_event_store(uint16_t port,
				     uint32_t event,
				     uint32_t state);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_wd_timeout_store(uint16_t port, uint32_t timeout);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_ver_show(uint16_t port, uint32_t *ver);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_wd_timeout_show(uint16_t port, uint32_t *wd_timeout);
 ...
 *   - (-EINVAL) if bad parameter.
 */
int rte_pmd_yusur2_bypass_wd_reset(uint16_t port);
 ...
 *   - (-ENOTSUP) if hardware doesn't support.
 *   - (-ENODEV) if *port* invalid.
 *   - (YUSUR2_ERR_SWFW_SYNC) If sw/fw semaphore acquisition failed
 */
__rte_experimental
int
 ...
 *   - (-ENOTSUP) if hardware doesn't support.
 *   - (-ENODEV) if *port* invalid.
 *   - (YUSUR2_ERR_PHY) If PHY read command failed
 */
__rte_experimental
int
...
/**
 * Response sent back to yusur2 driver from user app after callback
 */
enum rte_pmd_yusur2_mb_event_rsp {
	RTE_PMD_YUSUR2_MB_EVENT_NOOP_ACK,  /**< skip mbox request and ACK */
	RTE_PMD_YUSUR2_MB_EVENT_NOOP_NACK, /**< skip mbox request and NACK */
	RTE_PMD_YUSUR2_MB_EVENT_PROCEED,  /**< proceed with mbox request  */
	RTE_PMD_YUSUR2_MB_EVENT_MAX       /**< max value of this enum */
};

/**
 * Data sent to the user application when the callback is executed.
 */
struct rte_pmd_yusur2_mb_event_param {
	uint16_t vfid;     /**< Virtual Function number */
	uint16_t msg_type; /**< VF to PF message type, defined in yusur2_mbx.h */
	uint16_t retval;   /**< return value */
	void *msg;         /**< pointer to message */
};
enum {
	RTE_PMD_YUSUR2_BYPASS_MODE_NONE,
	RTE_PMD_YUSUR2_BYPASS_MODE_NORMAL,
	RTE_PMD_YUSUR2_BYPASS_MODE_BYPASS,
	RTE_PMD_YUSUR2_BYPASS_MODE_ISOLATE,
	RTE_PMD_YUSUR2_BYPASS_MODE_NUM,
};

#define RTE_PMD_YUSUR2_BYPASS_MODE_VALID(x)        \
	((x) > RTE_PMD_YUSUR2_BYPASS_MODE_NONE &&  \
	(x) < RTE_PMD_YUSUR2_BYPASS_MODE_NUM)

enum {
	RTE_PMD_YUSUR2_BYPASS_EVENT_NONE,
	RTE_PMD_YUSUR2_BYPASS_EVENT_START,
	RTE_PMD_YUSUR2_BYPASS_EVENT_OS_ON = RTE_PMD_YUSUR2_BYPASS_EVENT_START,
	RTE_PMD_YUSUR2_BYPASS_EVENT_POWER_ON,
	RTE_PMD_YUSUR2_BYPASS_EVENT_OS_OFF,
	RTE_PMD_YUSUR2_BYPASS_EVENT_POWER_OFF,
	RTE_PMD_YUSUR2_BYPASS_EVENT_TIMEOUT,
	RTE_PMD_YUSUR2_BYPASS_EVENT_NUM
};

#define RTE_PMD_YUSUR2_BYPASS_EVENT_VALID(x)       \
	((x) > RTE_PMD_YUSUR2_BYPASS_EVENT_NONE && \
	(x) < RTE_PMD_YUSUR2_BYPASS_MODE_NUM)

enum {
	RTE_PMD_YUSUR2_BYPASS_TMT_OFF,     /* timeout disabled. */
	RTE_PMD_YUSUR2_BYPASS_TMT_1_5_SEC, /* timeout for 1.5 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_2_SEC,   /* timeout for 2 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_3_SEC,   /* timeout for 3 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_4_SEC,   /* timeout for 4 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_8_SEC,   /* timeout for 8 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_16_SEC,  /* timeout for 16 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_32_SEC,  /* timeout for 32 seconds */
	RTE_PMD_YUSUR2_BYPASS_TMT_NUM
};

#define RTE_PMD_YUSUR2_BYPASS_TMT_VALID(x)       \
	((x) == RTE_PMD_YUSUR2_BYPASS_TMT_OFF || \
	((x) > RTE_PMD_YUSUR2_BYPASS_TMT_OFF &&  \
	(x) < RTE_PMD_YUSUR2_BYPASS_TMT_NUM))
