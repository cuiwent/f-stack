...
#include "yusur2_rxtx_vec_common.h"

#pragma GCC diagnostic ignored "-Wcast-qual"

static inline void
yusur2_rxq_rearm(struct yusur2_rx_queue *rxq)
{
	int i;
	uint16_t rx_id;
	volatile union yusur2_adv_rx_desc *rxdp;
	struct yusur2_rx_entry *rxep = &rxq->sw_ring[rxq->rxrearm_start];
	...
	if (unlikely(rte_mempool_get_bulk(rxq->mb_pool,
					  (void *)rxep,
					  RTE_YUSUR2_RXQ_REARM_THRESH) < 0)) {
		if (rxq->rxrearm_nb + RTE_YUSUR2_RXQ_REARM_THRESH >=
		    rxq->nb_rx_desc) {
			for (i = 0; i < RTE_YUSUR2_DESCS_PER_LOOP; i++) {
	...
	for (i = 0; i < RTE_YUSUR2_RXQ_REARM_THRESH; i += 2, rxep += 2) {
		mb0 = rxep[0].mbuf;
		mb1 = rxep[1].mbuf;
	...
	rxq->rxrearm_start += RTE_YUSUR2_RXQ_REARM_THRESH;
	if (rxq->rxrearm_start >= rxq->nb_rx_desc)
		rxq->rxrearm_start = 0;

	rxq->rxrearm_nb -= RTE_YUSUR2_RXQ_REARM_THRESH;
...
#define YUSUR2_VPMD_DESC_EOP_MASK	0x02020202
#define YUSUR2_UINT8_BIT			(CHAR_BIT * sizeof(uint8_t))
		...
		struct rte_mbuf **rx_pkts)
{
	...
	/* shift right by YUSUR2_PACKET_TYPE_SHIFT, and apply ptype mask */
	ptype0 = vandq_u32(vshrq_n_u32(ptype0, YUSUR2_PACKET_TYPE_SHIFT),
			ptype_mask);
...
/**
 * vPMD raw receive routine, only accept(nb_pkts >= RTE_YUSUR2_DESCS_PER_LOOP)
 *
 * Notice:
 * - nb_pkts < RTE_YUSUR2_DESCS_PER_LOOP, just return no packet
 * - floor align nb_pkts to a RTE_YUSUR2_DESC_PER_LOOP power-of-two
 */
static inline uint16_t
_recv_raw_pkts_vec(struct yusur2_rx_queue *rxq, struct rte_mbuf **rx_pkts,
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	/* nb_pkts has to be floor-aligned to RTE_YUSUR2_DESCS_PER_LOOP */
	nb_pkts = RTE_ALIGN_FLOOR(nb_pkts, RTE_YUSUR2_DESCS_PER_LOOP);
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
			pos += RTE_YUSUR2_DESCS_PER_LOOP,
			rxdp += RTE_YUSUR2_DESCS_PER_LOOP) {
		uint64x2_t descs[RTE_YUSUR2_DESCS_PER_LOOP];
		...
		if (split_packet) {
			...
			*(int *)split_packet = ~stat & YUSUR2_VPMD_DESC_EOP_MASK;

			split_packet += RTE_YUSUR2_DESCS_PER_LOOP;
		}

		/* C.4 expand DD bit to saturate UINT8 */
		staterr = vshlq_n_u8(staterr, YUSUR2_UINT8_BIT - 1);
		staterr = vreinterpretq_u8_s8
				(vshrq_n_s8(vreinterpretq_s8_u8(staterr),
					YUSUR2_UINT8_BIT - 1));
		stat = ~vgetq_lane_u32(vreinterpretq_u32_u8(staterr), 0);

		rte_prefetch_non_temporal(rxdp + RTE_YUSUR2_DESCS_PER_LOOP);
		...
		if (unlikely(stat == 0)) {
			nb_pkts_recd += RTE_YUSUR2_DESCS_PER_LOOP;
		} else {
			nb_pkts_recd += __builtin_ctz(stat) / YUSUR2_UINT8_BIT;
			break;
		}
	}
...
/**
 * vPMD receive routine, only accept(nb_pkts >= RTE_YUSUR2_DESCS_PER_LOOP)
 *
 * Notice:
 * - nb_pkts < RTE_YUSUR2_DESCS_PER_LOOP, just return no packet
 * - floor align nb_pkts to a RTE_YUSUR2_DESC_PER_LOOP power-of-two
 ...
 */
uint16_t
yusur2_recv_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
		uint16_t nb_pkts)
{
 ...
 */
static uint16_t
yusur2_recv_scattered_burst_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
			       uint16_t nb_pkts)
{
	struct yusur2_rx_queue *rxq = rx_queue;
	uint8_t split_flags[RTE_YUSUR2_MAX_RX_BURST] = {0};
 ...
 */
uint16_t
yusur2_recv_scattered_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
			      uint16_t nb_pkts)
{
	uint16_t retval = 0;

	while (nb_pkts > RTE_YUSUR2_MAX_RX_BURST) {
		uint16_t burst;

		burst = yusur2_recv_scattered_burst_vec(rx_queue,
						       rx_pkts + retval,
						       RTE_YUSUR2_MAX_RX_BURST);
		retval += burst;
		nb_pkts -= burst;
		if (burst < RTE_YUSUR2_MAX_RX_BURST)
			return retval;
	}

	return retval + yusur2_recv_scattered_burst_vec(rx_queue,
						       rx_pkts + retval,
						       nb_pkts);
}

static inline void
vtx1(volatile union yusur2_adv_tx_desc *txdp,
...
static inline void
vtx(volatile union yusur2_adv_tx_desc *txdp,
...
uint16_t
yusur2_xmit_fixed_burst_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
			   uint16_t nb_pkts)
{
	struct yusur2_tx_queue *txq = (struct yusur2_tx_queue *)tx_queue;
	volatile union yusur2_adv_tx_desc *txdp;
	struct yusur2_tx_entry_v *txep;
	uint16_t n, nb_commit, tx_id;
	uint64_t flags = DCMD_DTYP_FLAGS;
	uint64_t rs = YUSUR2_ADVTXD_DCMD_RS | DCMD_DTYP_FLAGS;
	int i;
	...
	YUSUR2_PCI_REG_WRITE(txq->tdt_reg_addr, txq->tx_tail);

	return nb_pkts;
}

static void __attribute__((cold))
yusur2_tx_queue_release_mbufs_vec(struct yusur2_tx_queue *txq)
{
	_yusur2_tx_queue_release_mbufs_vec(txq);
}

void __attribute__((cold))
yusur2_rx_queue_release_mbufs_vec(struct yusur2_rx_queue *rxq)
{
	_yusur2_rx_queue_release_mbufs_vec(rxq);
}

static void __attribute__((cold))
yusur2_tx_free_swring(struct yusur2_tx_queue *txq)
{
	_yusur2_tx_free_swring_vec(txq);
}

static void __attribute__((cold))
yusur2_reset_tx_queue(struct yusur2_tx_queue *txq)
{
	_yusur2_reset_tx_queue_vec(txq);
}

static const struct yusur2_txq_ops vec_txq_ops = {
	.release_mbufs = yusur2_tx_queue_release_mbufs_vec,
	.free_swring = yusur2_tx_free_swring,
	.reset = yusur2_reset_tx_queue,
};

int __attribute__((cold))
yusur2_rxq_vec_setup(struct yusur2_rx_queue *rxq)
{
	return yusur2_rxq_vec_setup_default(rxq);
}

int __attribute__((cold))
yusur2_txq_vec_setup(struct yusur2_tx_queue *txq)
{
	return yusur2_txq_vec_setup_default(txq, &vec_txq_ops);
}

int __attribute__((cold))
yusur2_rx_vec_dev_conf_condition_check(struct rte_eth_dev *dev)
{
	...
	return yusur2_rx_vec_dev_conf_condition_check_default(dev);
}
