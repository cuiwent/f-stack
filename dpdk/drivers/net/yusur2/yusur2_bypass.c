...
/**
 *  yusur2_bypass_set_time - Set bypass FW time epoc.
 *
 ...
 **/
static void
yusur2_bypass_set_time(struct yusur2_adapter *adapter)
{
	u32 mask, value;
	u32 sec;
	struct yusur2_hw *hw = &adapter->hw;
...
/**
 * yusur2_bypass_init - Make some environment changes for bypass
 *
 * @adapter: pointer to yusur2_adapter structure for access to state bits
 *
 * This function collects all the modifications needed by the bypass
 * driver.
 **/
void
yusur2_bypass_init(struct rte_eth_dev *dev)
{
	struct yusur2_adapter *adapter;
	struct yusur2_hw *hw;

	adapter = YUSUR2_DEV_TO_ADPATER(dev);
	hw = &adapter->hw;
	...
	/* set bypass ops. */
	adapter->bps.ops.bypass_rw = &yusur2_bypass_rw_generic;
	adapter->bps.ops.bypass_valid_rd = &yusur2_bypass_valid_rd_generic;
	adapter->bps.ops.bypass_set = &yusur2_bypass_set_generic;
	adapter->bps.ops.bypass_rd_eep = &yusur2_bypass_rd_eep_generic;

	/* set the time for logging. */
	yusur2_bypass_set_time(adapter);
...
s32
yusur2_bypass_state_show(struct rte_eth_dev *dev, u32 *state)
{
	struct yusur2_hw *hw;
	s32 ret_val;
	u32 cmd;
	u32 by_ctl = 0;
	struct yusur2_adapter *adapter = YUSUR2_DEV_TO_ADPATER(dev);
...
s32
yusur2_bypass_state_store(struct rte_eth_dev *dev, u32 *new_state)
{
	struct yusur2_adapter *adapter = YUSUR2_DEV_TO_ADPATER(dev);
	struct yusur2_hw *hw;
	s32 ret_val;
...
s32
yusur2_bypass_event_show(struct rte_eth_dev *dev, u32 event,
			    u32 *state)
{
	struct yusur2_hw *hw;
	s32 ret_val;
...
s32
yusur2_bypass_event_store(struct rte_eth_dev *dev, u32 event,
			     u32 state)
{
	struct yusur2_hw *hw;
	u32 status;
...
s32
yusur2_bypass_wd_timeout_store(struct rte_eth_dev *dev, u32 timeout)
{
	struct yusur2_hw *hw;
	u32 status;
	u32 mask;
	...
	if (timeout == RTE_PMD_YUSUR2_BYPASS_TMT_OFF) {
...
s32
yusur2_bypass_ver_show(struct rte_eth_dev *dev, u32 *ver)
{
	struct yusur2_hw *hw;
	u32 cmd;
	u32 status;
...
s32
yusur2_bypass_wd_timeout_show(struct rte_eth_dev *dev, u32 *wd_timeout)
{
	struct yusur2_hw *hw;
	u32 by_ctl = 0;
	u32 cmd;
	...
	if (!wdg)
		*wd_timeout = RTE_PMD_YUSUR2_BYPASS_TMT_OFF;
	else
...
s32
yusur2_bypass_wd_reset(struct rte_eth_dev *dev)
{
	u32 cmd;
	u32 status;
	u32 sec;
	...
	do {
		if (count++ > 10) {
			ret_val = YUSUR2_BYPASS_FW_WRITE_FAILURE;
			break;
		}

		if (adapter->bps.ops.bypass_rw(hw, BYPASS_PAGE_CTL1, &status)) {
			ret_val = YUSUR2_ERR_INVALID_ARGUMENT;
			break;
		}
