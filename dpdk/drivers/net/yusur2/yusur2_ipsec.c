...
#define RTE_YUSUR2_REGISTER_POLL_WAIT_5_MS  5

#define YUSUR2_WAIT_RREAD \
	YUSUR2_WRITE_REG_THEN_POLL_MASK(hw, YUSUR2_IPSRXIDX, reg_val, \
	IPSRXIDX_READ, RTE_YUSUR2_REGISTER_POLL_WAIT_5_MS)
#define YUSUR2_WAIT_RWRITE \
	YUSUR2_WRITE_REG_THEN_POLL_MASK(hw, YUSUR2_IPSRXIDX, reg_val, \
	IPSRXIDX_WRITE, RTE_YUSUR2_REGISTER_POLL_WAIT_5_MS)
#define YUSUR2_WAIT_TREAD \
	YUSUR2_WRITE_REG_THEN_POLL_MASK(hw, YUSUR2_IPSTXIDX, reg_val, \
	IPSRXIDX_READ, RTE_YUSUR2_REGISTER_POLL_WAIT_5_MS)
#define YUSUR2_WAIT_TWRITE \
	YUSUR2_WRITE_REG_THEN_POLL_MASK(hw, YUSUR2_IPSTXIDX, reg_val, \
	IPSRXIDX_WRITE, RTE_YUSUR2_REGISTER_POLL_WAIT_5_MS)
...
static void
yusur2_crypto_clear_ipsec_tables(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_ipsec *priv = YUSUR2_DEV_PRIVATE_TO_IPSEC(
	...
	for (i = 0; i < IPSEC_MAX_RX_IP_COUNT; i++) {
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(0), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(1), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(2), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(3), 0);
		YUSUR2_WAIT_RWRITE;
	}
	...
	for (i = 0; i < IPSEC_MAX_SA_COUNT; i++) {
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXSPI, 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPIDX, 0);
		YUSUR2_WAIT_RWRITE;
		reg_val = IPSRXIDX_WRITE | IPSRXIDX_TABLE_KEY | index;
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(0), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(1), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(2), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(3), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXSALT, 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXMOD, 0);
		YUSUR2_WAIT_RWRITE;
		reg_val = IPSRXIDX_WRITE | index;
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(0), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(1), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(2), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(3), 0);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXSALT, 0);
		YUSUR2_WAIT_TWRITE;
	}
...
static int
yusur2_crypto_add_sa(struct yusur2_crypto_session *ic_session)
{
	...
	if (ic_session->op == YUSUR2_OP_AUTHENTICATED_DECRYPTION) {
		int i, ip_index = -1;
		uint8_t *key;
		...
		if (priv->rx_ip_tbl[ip_index].ip.type == IPv4) {
			...
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(1), 0);
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(2), 0);
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(3),
					priv->rx_ip_tbl[ip_index].ip.ipv4);
		} else {
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(0),
					priv->rx_ip_tbl[ip_index].ip.ipv6[0]);
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(1),
					priv->rx_ip_tbl[ip_index].ip.ipv6[1]);
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(2),
					priv->rx_ip_tbl[ip_index].ip.ipv6[2]);
			YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPADDR(3),
					priv->rx_ip_tbl[ip_index].ip.ipv6[3]);
		}
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXSPI,
				priv->rx_sa_tbl[sa_index].spi);
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXIPIDX,
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(0),
			rte_cpu_to_be_32(*(uint32_t *)&key[12]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(1),
			rte_cpu_to_be_32(*(uint32_t *)&key[8]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(2),
			rte_cpu_to_be_32(*(uint32_t *)&key[4]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXKEY(3),
			rte_cpu_to_be_32(*(uint32_t *)&key[0]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXSALT,
				rte_cpu_to_be_32(ic_session->salt));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSRXMOD,
				priv->rx_sa_tbl[sa_index].mode);
		YUSUR2_WAIT_RWRITE;
		...
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(0),
			rte_cpu_to_be_32(*(uint32_t *)&key[12]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(1),
			rte_cpu_to_be_32(*(uint32_t *)&key[8]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(2),
			rte_cpu_to_be_32(*(uint32_t *)&key[4]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXKEY(3),
			rte_cpu_to_be_32(*(uint32_t *)&key[0]));
		YUSUR2_WRITE_REG(hw, YUSUR2_IPSTXSALT,
				rte_cpu_to_be_32(ic_session->salt));
		YUSUR2_WAIT_TWRITE;
...
static int
yusur2_crypto_remove_sa(struct rte_eth_dev *dev,
		       struct yusur2_crypto_session *ic_session)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct yusur2_ipsec *priv =
			YUSUR2_DEV_PRIVATE_TO_IPSEC(dev->data->dev_private);
	uint32_t reg_val;
	int sa_index = -1;
...
static int
yusur2_crypto_create_session(void *device,
		struct rte_security_session_conf *conf,
		struct rte_security_session *session,
		struct rte_mempool *mempool)
{
	struct rte_eth_dev *eth_dev = (struct rte_eth_dev *)device;
	struct yusur2_crypto_session *ic_session = NULL;
	...
	if (conf->ipsec.direction == RTE_SECURITY_IPSEC_SA_DIR_INGRESS) {
		if (dev_conf->rxmode.offloads & DEV_RX_OFFLOAD_SECURITY) {
			ic_session->op = YUSUR2_OP_AUTHENTICATED_DECRYPTION;
		} else {
		...
		if (dev_conf->txmode.offloads & DEV_TX_OFFLOAD_SECURITY) {
			ic_session->op = YUSUR2_OP_AUTHENTICATED_ENCRYPTION;
		} else {
	...
	if (ic_session->op == YUSUR2_OP_AUTHENTICATED_ENCRYPTION) {
		if (yusur2_crypto_add_sa(ic_session)) {
...
static unsigned int
yusur2_crypto_session_get_size(__rte_unused void *device)
{
	return sizeof(struct yusur2_crypto_session);
}

static int
yusur2_crypto_remove_session(void *device,
		struct rte_security_session *session)
{
	...
	if (yusur2_crypto_remove_sa(eth_dev, ic_session)) {
...
yusur2_crypto_compute_pad_len(struct rte_mbuf *m)
{
	if (m->nb_segs == 1) {
...
static int
yusur2_crypto_update_mb(void *device __rte_unused,
		struct rte_security_session *session,
		       struct rte_mbuf *m, void *params __rte_unused)
{
	...
	if (ic_session->op == YUSUR2_OP_AUTHENTICATED_ENCRYPTION) {
		union yusur2_crypto_tx_desc_md *mdata =
			(union yusur2_crypto_tx_desc_md *)&m->udata64;
		mdata->enc = 1;
		mdata->sa_idx = ic_session->sa_index;
		mdata->pad_len = yusur2_crypto_compute_pad_len(m);
	}
	return 0;
}
...
yusur2_crypto_capabilities_get(void *device __rte_unused)
{
	...
	return yusur2_security_capabilities;
}


int
yusur2_crypto_enable_ipsec(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t reg;
	...
	/* Set YUSUR2_SECTXBUFFAF to 0x15 as required in the datasheet*/
	YUSUR2_WRITE_REG(hw, YUSUR2_SECTXBUFFAF, 0x15);
	...
	reg  = YUSUR2_READ_REG(hw, YUSUR2_HLREG0);
	reg |= YUSUR2_HLREG0_TXCRCEN | YUSUR2_HLREG0_RXCRCSTRP;
	YUSUR2_WRITE_REG(hw, YUSUR2_HLREG0, reg);

	if (rx_offloads & DEV_RX_OFFLOAD_SECURITY) {
		YUSUR2_WRITE_REG(hw, YUSUR2_SECRXCTRL, 0);
		reg = YUSUR2_READ_REG(hw, YUSUR2_SECRXCTRL);
		if (reg != 0) {
	...
	if (tx_offloads & DEV_TX_OFFLOAD_SECURITY) {
		YUSUR2_WRITE_REG(hw, YUSUR2_SECTXCTRL,
				YUSUR2_SECTXCTRL_STORE_FORWARD);
		reg = YUSUR2_READ_REG(hw, YUSUR2_SECTXCTRL);
		if (reg != YUSUR2_SECTXCTRL_STORE_FORWARD) {
			PMD_DRV_LOG(ERR, "Error enabling Rx Crypto");
			return -1;
		}
	}

	yusur2_crypto_clear_ipsec_tables(dev);

	return 0;
}

int
yusur2_crypto_add_ingress_sa_from_flow(const void *sess,
				      const void *ip_spec,
				      uint8_t is_ipv6)
{
	struct yusur2_crypto_session *ic_session
	...
	if (ic_session->op == YUSUR2_OP_AUTHENTICATED_DECRYPTION) {
		...
		return yusur2_crypto_add_sa(ic_session);
	}

	return 0;
}

static struct rte_security_ops yusur2_security_ops = {
	.session_create = yusur2_crypto_create_session,
	.session_update = NULL,
	.session_get_size = yusur2_crypto_session_get_size,
	.session_stats_get = NULL,
	.session_destroy = yusur2_crypto_remove_session,
	.set_pkt_metadata = yusur2_crypto_update_mb,
	.capabilities_get = yusur2_crypto_capabilities_get
};

static int
yusur2_crypto_capable(struct rte_eth_dev *dev)
{
	struct yusur2_hw *hw = YUSUR2_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t reg_i, reg, capable = 1;
	/* test if rx crypto can be enabled and then write back initial value*/
	reg_i = YUSUR2_READ_REG(hw, YUSUR2_SECRXCTRL);
	YUSUR2_WRITE_REG(hw, YUSUR2_SECRXCTRL, 0);
	reg = YUSUR2_READ_REG(hw, YUSUR2_SECRXCTRL);
	if (reg != 0)
		capable = 0;
	YUSUR2_WRITE_REG(hw, YUSUR2_SECRXCTRL, reg_i);
	return capable;
}

int
yusur2_ipsec_ctx_create(struct rte_eth_dev *dev)
{
	struct rte_security_ctx *ctx = NULL;

	if (yusur2_crypto_capable(dev)) {
		ctx = rte_malloc("rte_security_instances_ops",
				 sizeof(struct rte_security_ctx), 0);
		if (ctx) {
			ctx->device = (void *)dev;
			ctx->ops = &yusur2_security_ops;
			ctx->sess_cnt = 0;
