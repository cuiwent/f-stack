...
#define YUSUR2_MIN_N_TUPLE_PRIO 1
#define YUSUR2_MAX_N_TUPLE_PRIO 7
#define YUSUR2_MAX_FLX_SOURCE_OFF 62

/* ntuple filter list structure */
struct yusur2_ntuple_filter_ele {
	TAILQ_ENTRY(yusur2_ntuple_filter_ele) entries;
	struct rte_eth_ntuple_filter filter_info;
};
/* ethertype filter list structure */
struct yusur2_ethertype_filter_ele {
	TAILQ_ENTRY(yusur2_ethertype_filter_ele) entries;
	struct rte_eth_ethertype_filter filter_info;
};
/* syn filter list structure */
struct yusur2_eth_syn_filter_ele {
	TAILQ_ENTRY(yusur2_eth_syn_filter_ele) entries;
	struct rte_eth_syn_filter filter_info;
};
/* fdir filter list structure */
struct yusur2_fdir_rule_ele {
	TAILQ_ENTRY(yusur2_fdir_rule_ele) entries;
	struct yusur2_fdir_rule filter_info;
};
/* l2_tunnel filter list structure */
struct yusur2_eth_l2_tunnel_conf_ele {
	TAILQ_ENTRY(yusur2_eth_l2_tunnel_conf_ele) entries;
	struct rte_eth_l2_tunnel_conf filter_info;
};
/* rss filter list structure */
struct yusur2_rss_conf_ele {
	TAILQ_ENTRY(yusur2_rss_conf_ele) entries;
	struct yusur2_rte_flow_rss_conf filter_info;
};
/* yusur2_flow memory list structure */
struct yusur2_flow_mem {
	TAILQ_ENTRY(yusur2_flow_mem) entries;
	struct rte_flow *flow;
};

TAILQ_HEAD(yusur2_ntuple_filter_list, yusur2_ntuple_filter_ele);
TAILQ_HEAD(yusur2_ethertype_filter_list, yusur2_ethertype_filter_ele);
TAILQ_HEAD(yusur2_syn_filter_list, yusur2_eth_syn_filter_ele);
TAILQ_HEAD(yusur2_fdir_rule_filter_list, yusur2_fdir_rule_ele);
TAILQ_HEAD(yusur2_l2_tunnel_filter_list, yusur2_eth_l2_tunnel_conf_ele);
TAILQ_HEAD(yusur2_rss_filter_list, yusur2_rss_conf_ele);
TAILQ_HEAD(yusur2_flow_mem_list, yusur2_flow_mem);

static struct yusur2_ntuple_filter_list filter_ntuple_list;
static struct yusur2_ethertype_filter_list filter_ethertype_list;
static struct yusur2_syn_filter_list filter_syn_list;
static struct yusur2_fdir_rule_filter_list filter_fdir_list;
static struct yusur2_l2_tunnel_filter_list filter_l2_tunnel_list;
static struct yusur2_rss_filter_list filter_rss_list;
static struct yusur2_flow_mem_list yusur2_flow_list;
			 ...
			 struct rte_flow_error *error)
{
	...
	if (act->type == RTE_FLOW_ACTION_TYPE_SECURITY) {
		...
		return yusur2_crypto_add_ingress_sa_from_flow(conf, item->spec,
					item->type == RTE_FLOW_ITEM_TYPE_IPV6);
	}
#endif
...
action:
	...
	if (attr->priority < YUSUR2_MIN_N_TUPLE_PRIO ||
	    attr->priority > YUSUR2_MAX_N_TUPLE_PRIO)
	    filter->priority = 1;

	return 0;
}

/* a specific function for yusur2 because the flags is specific */
static int
yusur2_parse_ntuple_filter(struct rte_eth_dev *dev,
			  ...
			  struct rte_flow_error *error)
{
	...
	if (filter->priority < YUSUR2_MIN_N_TUPLE_PRIO ||
	    filter->priority > YUSUR2_MAX_N_TUPLE_PRIO) {
		memset(filter, 0, sizeof(struct rte_eth_ntuple_filter));
		rte_flow_error_set(error, EINVAL,
			RTE_FLOW_ERROR_TYPE_ITEM,
...
static int
yusur2_parse_ethertype_filter(struct rte_eth_dev *dev,
				 const struct rte_flow_attr *attr,
			     const struct rte_flow_item pattern[],
			     const struct rte_flow_action actions[],
			     struct rte_eth_ethertype_filter *filter,
...
static int
yusur2_parse_syn_filter(struct rte_eth_dev *dev,
				 const struct rte_flow_attr *attr,
			     const struct rte_flow_item pattern[],
			     const struct rte_flow_action actions[],
			     struct rte_eth_syn_filter *filter,
...
static int
yusur2_parse_l2_tn_filter(struct rte_eth_dev *dev,
			const struct rte_flow_attr *attr,
			const struct rte_flow_item pattern[],
			const struct rte_flow_action actions[],
...
static int
yusur2_parse_fdir_act_attr(const struct rte_flow_attr *attr,
			  const struct rte_flow_action actions[],
 ...
 */
static int
yusur2_parse_fdir_filter_normal(struct rte_eth_dev *dev,
			       const struct rte_flow_attr *attr,
			       const struct rte_flow_item pattern[],
			       const struct rte_flow_action actions[],
			       struct yusur2_fdir_rule *rule,
			       struct rte_flow_error *error)
{
	 ...
	 */
	memset(rule, 0, sizeof(struct yusur2_fdir_rule));
	memset(&rule->mask, 0xFF, sizeof(struct yusur2_hw_fdir_mask));
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
		...
		if (item->spec) {
			...
			for (j = 0; j < RTE_ETHER_ADDR_LEN; j++) {
				rule->yusur2_fdir.formatted.inner_mac[j] =
		...
		if (item->mask) {
			...
			for (j = 0; j < RTE_ETHER_ADDR_LEN; j++) {
				if (eth_mask->src.addr_bytes[j] ||
					...
					memset(rule, 0,
					sizeof(struct yusur2_fdir_rule));
					rte_flow_error_set(error, EINVAL,
					RTE_FLOW_ERROR_TYPE_ITEM,
					item, "Not supported by fdir filter");
					return -rte_errno;
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_VLAN) {
		...
		rule->yusur2_fdir.formatted.vlan_id = vlan_spec->tci;
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_IPV4) {
		...
		if (item->spec) {
			rule->b_spec = TRUE;
			ipv4_spec = item->spec;
			rule->yusur2_fdir.formatted.dst_ip[0] =
				ipv4_spec->hdr.dst_addr;
			rule->yusur2_fdir.formatted.src_ip[0] =
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_IPV6) {
		...
		if (item->spec) {
			rule->b_spec = TRUE;
			ipv6_spec = item->spec;
			rte_memcpy(rule->yusur2_fdir.formatted.src_ip,
				   ipv6_spec->hdr.src_addr, 16);
			rte_memcpy(rule->yusur2_fdir.formatted.dst_ip,
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_TCP) {
		...
		if (item->spec) {
			rule->b_spec = TRUE;
			tcp_spec = item->spec;
			rule->yusur2_fdir.formatted.src_port =
				tcp_spec->hdr.src_port;
			rule->yusur2_fdir.formatted.dst_port =
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_RAW) {
		...
		if (raw_spec->relative != 0 ||
		    raw_spec->search != 0 ||
		    raw_spec->reserved != 0 ||
		    raw_spec->offset > YUSUR2_MAX_FLX_SOURCE_OFF ||
		    raw_spec->offset % 2 ||
		    raw_spec->limit != 0 ||
		    raw_spec->length != 2 ||
		...
		rule->yusur2_fdir.formatted.flex_bytes =
	...
	return yusur2_parse_fdir_act_attr(attr, actions, rule, error);
}
 ...
 */
static int
yusur2_parse_fdir_filter_tunnel(const struct rte_flow_attr *attr,
			       const struct rte_flow_item pattern[],
			       const struct rte_flow_action actions[],
			       struct yusur2_fdir_rule *rule,
			       struct rte_flow_error *error)
{
	...
	/* Get the VxLAN info */
	if (item->type == RTE_FLOW_ITEM_TYPE_VXLAN) {
		rule->yusur2_fdir.formatted.tunnel_type =
		...
		if (item->spec) {
			...
			rte_memcpy(((uint8_t *)
				&rule->yusur2_fdir.formatted.tni_vni),
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_NVGRE) {
		...
		if (item->spec) {
			...
			/* tni is a 24-bits bit field */
			rte_memcpy(&rule->yusur2_fdir.formatted.tni_vni,
...
static int
yusur2_parse_fdir_filter(struct rte_eth_dev *dev,
			const struct rte_flow_attr *attr,
			const struct rte_flow_item pattern[],
			const struct rte_flow_action actions[],
			struct yusur2_fdir_rule *rule,
			struct rte_flow_error *error)
{
	...
	ret = yusur2_parse_fdir_filter_normal(dev, attr, pattern,
					actions, rule, error);

	if (!ret)
		goto step_next;

	ret = yusur2_parse_fdir_filter_tunnel(attr, pattern,
					actions, rule, error);
...
step_next:
	...
	if (hw->mac.type == yusur2_mac_82599EB &&
		rule->fdirflags == YUSUR2_FDIRCMD_DROP &&
		(rule->yusur2_fdir.formatted.src_port != 0 ||
		rule->yusur2_fdir.formatted.dst_port != 0))
		return -ENOTSUP;
...
static int
yusur2_parse_rss_filter(struct rte_eth_dev *dev,
			const struct rte_flow_attr *attr,
			const struct rte_flow_action actions[],
			struct yusur2_rte_flow_rss_conf *rss_conf,
			struct rte_flow_error *error)
{
	const struct rte_flow_action *act;
	...
	if (act->type != RTE_FLOW_ACTION_TYPE_RSS) {
		memset(rss_conf, 0, sizeof(struct yusur2_rte_flow_rss_conf));
		rte_flow_error_set(error, EINVAL,
			RTE_FLOW_ERROR_TYPE_ACTION,
			act, "Not supported action.");
		return -rte_errno;
	...
	if (yusur2_rss_conf_init(rss_conf, rss))
		return rte_flow_error_set
...
static void
yusur2_clear_rss_filter(struct rte_eth_dev *dev)
{
	struct yusur2_filter_info *filter_info =
		YUSUR2_DEV_PRIVATE_TO_FILTER_INFO(dev->data->dev_private);

	if (filter_info->rss_info.conf.queue_num)
		yusur2_config_rss_filter(dev, &filter_info->rss_info, FALSE);
}

void
yusur2_filterlist_init(void)
{
	...
	TAILQ_INIT(&yusur2_flow_list);
}

void
yusur2_filterlist_flush(void)
{
	struct yusur2_ntuple_filter_ele *ntuple_filter_ptr;
	struct yusur2_ethertype_filter_ele *ethertype_filter_ptr;
	struct yusur2_eth_syn_filter_ele *syn_filter_ptr;
	struct yusur2_eth_l2_tunnel_conf_ele *l2_tn_filter_ptr;
	struct yusur2_fdir_rule_ele *fdir_rule_ptr;
	struct yusur2_flow_mem *yusur2_flow_mem_ptr;
	struct yusur2_rss_conf_ele *rss_filter_ptr;
	...
	while ((yusur2_flow_mem_ptr = TAILQ_FIRST(&yusur2_flow_list))) {
		TAILQ_REMOVE(&yusur2_flow_list,
				 yusur2_flow_mem_ptr,
				 entries);
		rte_free(yusur2_flow_mem_ptr->flow);
		rte_free(yusur2_flow_mem_ptr);
	}
}
...
yusur2_flow_create(struct rte_eth_dev *dev,
		  const struct rte_flow_attr *attr,
		  const struct rte_flow_item pattern[],
		  const struct rte_flow_action actions[],
		  struct rte_flow_error *error)
{
	...
	struct yusur2_fdir_rule fdir_rule;
	struct rte_eth_l2_tunnel_conf l2_tn_filter;
	...
	flow = rte_zmalloc("yusur2_rte_flow", sizeof(struct rte_flow), 0);
	if (!flow) {
		PMD_DRV_LOG(ERR, "failed to allocate memory");
		return (struct rte_flow *)flow;
	}
	yusur2_flow_mem_ptr = rte_zmalloc("yusur2_flow_mem",
			sizeof(struct yusur2_flow_mem), 0);
	if (!yusur2_flow_mem_ptr) {
		PMD_DRV_LOG(ERR, "failed to allocate memory");
		rte_free(flow);
		return NULL;
	}
	yusur2_flow_mem_ptr->flow = flow;
	TAILQ_INSERT_TAIL(&yusur2_flow_list,
				yusur2_flow_mem_ptr, entries);

	memset(&ntuple_filter, 0, sizeof(struct rte_eth_ntuple_filter));
	ret = yusur2_parse_ntuple_filter(dev, attr, pattern,
	...
	if (!ret) {
		ret = yusur2_add_del_ntuple_filter(dev, &ntuple_filter, TRUE);
		if (!ret) {
			ntuple_filter_ptr = rte_zmalloc("yusur2_ntuple_filter",
				sizeof(struct yusur2_ntuple_filter_ele), 0);
	...
	ret = yusur2_parse_ethertype_filter(dev, attr, pattern,
	...
	if (!ret) {
		...
		if (!ret) {
			ethertype_filter_ptr = rte_zmalloc(
				"yusur2_ethertype_filter",
				sizeof(struct yusur2_ethertype_filter_ele), 0);
	...
	ret = yusur2_parse_syn_filter(dev, attr, pattern,
				actions, &syn_filter, error);
	if (!ret) {
		ret = yusur2_syn_filter_set(dev, &syn_filter, TRUE);
		if (!ret) {
			syn_filter_ptr = rte_zmalloc("yusur2_syn_filter",
				sizeof(struct yusur2_eth_syn_filter_ele), 0);
			if (!syn_filter_ptr) {
				PMD_DRV_LOG(ERR, "failed to allocate memory");
				goto out;
			}
	...
	memset(&fdir_rule, 0, sizeof(struct yusur2_fdir_rule));
	ret = yusur2_parse_fdir_filter(dev, attr, pattern,
	...
	if (!ret) {
		...
		if (fdir_rule.b_mask) {
			if (!fdir_info->mask_added) {
				...
				rte_memcpy(&fdir_info->mask,
					&fdir_rule.mask,
					sizeof(struct yusur2_hw_fdir_mask));
				fdir_info->flex_bytes_offset =
					fdir_rule.flex_bytes_offset;

				if (fdir_rule.mask.flex_bytes_mask)
					yusur2_fdir_set_flexbytes_offset(dev,
						fdir_rule.flex_bytes_offset);

				ret = yusur2_fdir_set_input_mask(dev);
				if (ret)
					goto out;
		...
		if (fdir_rule.b_spec) {
			ret = yusur2_fdir_filter_program(dev, &fdir_rule,
					FALSE, FALSE);
			if (!ret) {
				fdir_rule_ptr = rte_zmalloc("yusur2_fdir_filter",
					sizeof(struct yusur2_fdir_rule_ele), 0);
				if (!fdir_rule_ptr) {
					PMD_DRV_LOG(ERR, "failed to allocate memory");
					goto out;
				}
	...
	memset(&l2_tn_filter, 0, sizeof(struct rte_eth_l2_tunnel_conf));
	ret = yusur2_parse_l2_tn_filter(dev, attr, pattern,
					actions, &l2_tn_filter, error);
	if (!ret) {
		ret = yusur2_dev_l2_tunnel_filter_add(dev, &l2_tn_filter, FALSE);
		if (!ret) {
			l2_tn_filter_ptr = rte_zmalloc("yusur2_l2_tn_filter",
				sizeof(struct yusur2_eth_l2_tunnel_conf_ele), 0);
			if (!l2_tn_filter_ptr) {
				PMD_DRV_LOG(ERR, "failed to allocate memory");
				goto out;
			}
	...
	memset(&rss_conf, 0, sizeof(struct yusur2_rte_flow_rss_conf));
	ret = yusur2_parse_rss_filter(dev, attr,
					actions, &rss_conf, error);
	if (!ret) {
		ret = yusur2_config_rss_filter(dev, &rss_conf, TRUE);
		if (!ret) {
			rss_filter_ptr = rte_zmalloc("yusur2_rss_filter",
				sizeof(struct yusur2_rss_conf_ele), 0);
			if (!rss_filter_ptr) {
				PMD_DRV_LOG(ERR, "failed to allocate memory");
				goto out;
			}
			yusur2_rss_conf_init(&rss_filter_ptr->filter_info,
					    &rss_conf.conf);
...
/**
 * Check if the flow rule is supported by yusur2.
 * It only checkes the format. Don't guarantee the rule can be programmed into
 * the HW. Because there can be no enough room for the rule.
 */
static int
yusur2_flow_validate(struct rte_eth_dev *dev,
		const struct rte_flow_attr *attr,
		const struct rte_flow_item pattern[],
		const struct rte_flow_action actions[],
		struct rte_flow_error *error)
...
/* Destroy a flow rule on yusur2. */
static int
yusur2_flow_destroy(struct rte_eth_dev *dev,
		struct rte_flow *flow,
		struct rte_flow_error *error)
{
	int ret;
	...
	case RTE_ETH_FILTER_NTUPLE:
		ntuple_filter_ptr = (struct yusur2_ntuple_filter_ele *)
					pmd_flow->rule;
		rte_memcpy(&ntuple_filter,
			&ntuple_filter_ptr->filter_info,
			sizeof(struct rte_eth_ntuple_filter));
		ret = yusur2_add_del_ntuple_filter(dev, &ntuple_filter, FALSE);
		if (!ret) {
			TAILQ_REMOVE(&filter_ntuple_list,
			ntuple_filter_ptr, entries);
			rte_free(ntuple_filter_ptr);
	...
	case RTE_ETH_FILTER_ETHERTYPE:
		ethertype_filter_ptr = (struct yusur2_ethertype_filter_ele *)
					pmd_flow->rule;
	...
	case RTE_ETH_FILTER_SYN:
		syn_filter_ptr = (struct yusur2_eth_syn_filter_ele *)
				pmd_flow->rule;
		rte_memcpy(&syn_filter,
			&syn_filter_ptr->filter_info,
			sizeof(struct rte_eth_syn_filter));
		ret = yusur2_syn_filter_set(dev, &syn_filter, FALSE);
		if (!ret) {
			TAILQ_REMOVE(&filter_syn_list,
	...
	case RTE_ETH_FILTER_FDIR:
		fdir_rule_ptr = (struct yusur2_fdir_rule_ele *)pmd_flow->rule;
		rte_memcpy(&fdir_rule,
			&fdir_rule_ptr->filter_info,
			sizeof(struct yusur2_fdir_rule));
		ret = yusur2_fdir_filter_program(dev, &fdir_rule, TRUE, FALSE);
		if (!ret) {
			TAILQ_REMOVE(&filter_fdir_list,
				fdir_rule_ptr, entries);
			rte_free(fdir_rule_ptr);
	...
	case RTE_ETH_FILTER_L2_TUNNEL:
		l2_tn_filter_ptr = (struct yusur2_eth_l2_tunnel_conf_ele *)
				pmd_flow->rule;
		rte_memcpy(&l2_tn_filter, &l2_tn_filter_ptr->filter_info,
			sizeof(struct rte_eth_l2_tunnel_conf));
		ret = yusur2_dev_l2_tunnel_filter_del(dev, &l2_tn_filter);
		if (!ret) {
			TAILQ_REMOVE(&filter_l2_tunnel_list,
				l2_tn_filter_ptr, entries);
			rte_free(l2_tn_filter_ptr);
	...
	case RTE_ETH_FILTER_HASH:
		rss_filter_ptr = (struct yusur2_rss_conf_ele *)
				pmd_flow->rule;
		ret = yusur2_config_rss_filter(dev,
	...
	TAILQ_FOREACH(yusur2_flow_mem_ptr, &yusur2_flow_list, entries) {
		if (yusur2_flow_mem_ptr->flow == pmd_flow) {
			TAILQ_REMOVE(&yusur2_flow_list,
				yusur2_flow_mem_ptr, entries);
			rte_free(yusur2_flow_mem_ptr);
		}
...
/*  Destroy all flow rules associated with a port on yusur2. */
static int
yusur2_flow_flush(struct rte_eth_dev *dev,
		struct rte_flow_error *error)
{
	int ret = 0;

	yusur2_clear_all_ntuple_filter(dev);
	yusur2_clear_all_ethertype_filter(dev);
	yusur2_clear_syn_filter(dev);

	ret = yusur2_clear_all_fdir_filter(dev);
	if (ret < 0) {
	...
	ret = yusur2_clear_all_l2_tn_filter(dev);
	if (ret < 0) {
		rte_flow_error_set(error, EINVAL, RTE_FLOW_ERROR_TYPE_HANDLE,
					NULL, "Failed to flush rule");
		return ret;
...
const struct rte_flow_ops yusur2_flow_ops = {
	.validate = yusur2_flow_validate,
	.create = yusur2_flow_create,
	.destroy = yusur2_flow_destroy,
	.flush = yusur2_flow_flush,
};
