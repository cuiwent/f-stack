...
desc_to_olflags_v_ipsec(__m128i descs[4], struct rte_mbuf **rx_pkts)
{
	...
	const __m128i ipsec_sterr_msk =
			_mm_set1_epi32(YUSUR2_RXDADV_IPSEC_STATUS_SECP |
				       YUSUR2_RXDADV_IPSEC_ERROR_AUTH_FAILED);
	...
	struct rte_mbuf **rx_pkts)
{
	...
	const __m128i vlan_csum_msk = _mm_set_epi16(
		(YUSUR2_RXDADV_ERR_TCPE | YUSUR2_RXDADV_ERR_IPE) >> 16,
		(YUSUR2_RXDADV_ERR_TCPE | YUSUR2_RXDADV_ERR_IPE) >> 16,
		(YUSUR2_RXDADV_ERR_TCPE | YUSUR2_RXDADV_ERR_IPE) >> 16,
		(YUSUR2_RXDADV_ERR_TCPE | YUSUR2_RXDADV_ERR_IPE) >> 16,
		YUSUR2_RXD_STAT_VP, YUSUR2_RXD_STAT_VP,
		YUSUR2_RXD_STAT_VP, YUSUR2_RXD_STAT_VP);
...
{
	if (etqf_check & (0x02 << (index * RTE_YUSUR2_DESCS_PER_LOOP)))
		return RTE_PTYPE_UNKNOWN;

	if (tunnel_check & (0x02 << (index * RTE_YUSUR2_DESCS_PER_LOOP))) {
		...
		struct rte_mbuf **rx_pkts)
{
	...
	/* shift left by YUSUR2_PACKET_TYPE_SHIFT, and apply ptype mask */
	ptype0 = _mm_and_si128(_mm_srli_epi32(ptype0, YUSUR2_PACKET_TYPE_SHIFT),
			       ptype_mask);
		...
		uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		__m128i descs[RTE_YUSUR2_DESCS_PER_LOOP];
		...
		nb_pkts_recd += var;
		if (likely(var != RTE_YUSUR2_DESCS_PER_LOOP))
			break;
	}
